...
public class DatabaseHelper {
	...
	// the last column contains 1 (true) if the timestamp is less than 5 minutes old. 0 otherwise.
	// Method strftime(..) returns time in seconds, timestamp is in milliseconds
	private static final String[] LAST_DATA_PROJECTION = new String[] {
            SensorContract.Sensor._ID,
            SensorContract.Sensor.ADDRESS, SensorContract.Sensor.NAME,
            SensorContract.Sensor.Data.TIMESTAMP,
			SensorContract.Sensor.Data.FREQ,
            SensorContract.Sensor.Data.TOTAL_FREQ,
            SensorContract.Sensor.Data.BATTERY,
			SensorContract.Sensor.Data.TIMESTAMP +
                    " > 1000 * strftime('%s','now', '-5 minutes') as"
                    + SensorContract.Sensor.Data.RECENT };

	private static final String[] DATA_PROJECTION = new String[] {
            SensorContract.Sensor.Data._ID,
            SensorContract.Sensor.Data.TIMESTAMP,
            SensorContract.Sensor.Data.FREQ,
            SensorContract.Sensor.Data.TOTAL_FREQ,
			SensorContract.Sensor.Data.BATTERY };

	private static final String[] SENSOR_ID_PROJECTION = new String[] {
            SensorContract.Sensor._ID };

	private static final String[] SENSOR_ID_STORED_PROJECTION = new String[] {
            SensorContract.Sensor._ID,
            SensorContract.Sensor.STORED };
	...
	public Cursor getAllStoredSensors() {
		return mDatabase.query(Views.SENSOR_LAST_DATA,
                LAST_DATA_PROJECTION,
                ONLY_STORED,
                null,
                null,
                null,
                SensorContract.Sensor.Data.RECENT + " DESC," + SensorContract.Sensor.NAME + " ASC");
	}
	 ...
	 * @return the row ID of the newly inserted row, or -1 if an error occurred
	 */
	private long addSensorData(final long sensorId, final long timestamp, final double freq, final double totalfreq, final int battery) {
		final ContentValues values = new ContentValues();
		values.put(SensorContract.Sensor.Data.SENSOR_ID, sensorId);
		values.put(SensorContract.Sensor.Data.TIMESTAMP, timestamp);
		values.put(SensorContract.Sensor.Data.FREQ, freq);
        values.put(SensorContract.Sensor.Data.TOTAL_FREQ, totalfreq);
	 ...
	 * @return the row ID of the newly inserted row, or -1 if an error occurred
	 */
	private long updateSensorData(final long sensorId, final long timestamp, final double freq, final double totalfreq, final int battery) {
		final String selection = SensorContract.Sensor._ID + "=?";
		mSingleArg[0] = String.valueOf(sensorId);
	 ...
	 */
	public long addNewSensorData(final String address, final String name, final long timestamp, final double freq, final double totalfreq, final int battery) {
		final String selection = SensorContract.Sensor.ADDRESS + "=?";
		mSingleArg[0] = address;
		...
		try {
			...
			if (!cursor.moveToNext()) {
				...
				return addSensorData(id, timestamp, freq, totalfreq, battery);
			} else {
				id = cursor.getLong(0 /* _ID */);
				...
				if (stored)
					...
					return updateSensorData(id, timestamp, freq, totalfreq, battery);
			}
		} finally {
			cursor.close();
		}
	...
	private class SQLiteHelper extends SQLiteOpenHelper {
		 ...
		 * </pre>
		 */
		private static final String CREATE_SENSORS =
                "CREATE TABLE " + Tables.SENSORS + "(" +
                SensorContract.Sensor._ID + " INTEGER PRIMARY KEY AUTOINCREMENT, " +
                SensorContract.Sensor.ADDRESS+ " TEXT NOT NULL, " +
                SensorContract.Sensor.NAME + " TEXT, " +
                SensorContract.Sensor.STORED + " INT NOT NULL DEFAULT(0));";
		 ...
		 * |                                                   sensor_data                                                    |
		 * --------------------------------------------------------------------------------------------------------------------
		 * | _id (int, pk, auto increment) | sensor_id (int, fk->sensors(_id) | timestamp (int) | freq (real) | total_freq (real) | battery (int) |
		 * --------------------------------------------------------------------------------------------------------------------
		 * </pre>
		 */
		private static final String CREATE_DATA = "CREATE TABLE " + Tables.SENSOR_DATA + "(" +
                SensorContract.Sensor.Data._ID + " INTEGER PRIMARY KEY AUTOINCREMENT, " +
                SensorContract.Sensor.Data.SENSOR_ID + " INTEGER NOT NULL," + SensorContract.Sensor.Data.TIMESTAMP + " INTEGER NOT NULL, " +
                SensorContract.Sensor.Data.FREQ + " REAL, " +
                SensorContract.Sensor.Data.TOTAL_FREQ + " REAL, " +
                SensorContract.Sensor.Data.BATTERY + " INTEGER, FOREIGN KEY(" + SensorContract.Sensor.Data.SENSOR_ID + ") REFERENCES " +
                Tables.SENSORS + "(" + SensorContract.Sensor._ID + "));";
		 ...
		 * |                            (sensors)                                  |                   (sensor_data)               |
		 * ------------------------------------------------------------------------------------------------------------------------
		 * | _id (int, pk, auto increment) | address (text) | name (text) | stored | timestamp (int) | total_freq (real) | battery (int) |
		 * -------------------------------------------------------------------------------------------------------------------------
		 * </pre>
		 */
		private static final String CREATE_LAST_DATA_VIEW =
                "CREATE VIEW " + Views.SENSOR_LAST_DATA
                + " AS SELECT " + SensorContract.Sensor._ID + ","
                + SensorContract.Sensor.ADDRESS + ","
				+ SensorContract.Sensor.NAME + ","
                + SensorContract.Sensor.STORED + ",newest."
                + SensorContract.Sensor.Data.TIMESTAMP + " AS " + SensorContract.Sensor.Data.TIMESTAMP + ",newest."
				+ SensorContract.Sensor.Data.TOTAL_FREQ + " AS " + SensorContract.Sensor.Data.TOTAL_FREQ + ",newest."
                + SensorContract.Sensor.Data.BATTERY + " AS " + SensorContract.Sensor.Data.BATTERY +

                " INNER JOIN (SELECT " + SensorContract.Sensor.Data.SENSOR_ID + ","
				+ SensorContract.Sensor.Data.TIMESTAMP + ","
                + SensorContract.Sensor.Data.TOTAL_FREQ + ","
                + SensorContract.Sensor.Data.BATTERY
                + " FROM " + Tables.SENSOR_DATA
                + " GROUP BY "+ SensorContract.Sensor.Data.SENSOR_ID
                + " ORDER BY " + SensorContract.Sensor.Data.TIMESTAMP
                + " DESC) newest ON " + Tables.SENSORS + "."
                + SensorContract.Sensor._ID + "=newest."
